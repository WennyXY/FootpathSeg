...
import operator
from tile2net_utils.geodata_utils import geo2geodf
import collections
from functools import reduce
from tile2net_utils.momepy_shapes import *
...
def make_longer(self, line, thr):
    ...
    line_checked = validate_linemerge(line)
    extended_checked = validate_linemerge(extended)
...
def clean_deadend_dangles(gdf, dang_trh=25, dead_trh=18):
    df = gdf.reset_index(drop=True).explode(index_parts=True).reset_index(drop=True)
...
def remove_false_nodes(gdf):
    ...
    if len(merge) > 0:
        ...
        # else:
        final = gpd.GeoSeries(new).explode(index_parts=True).reset_index(drop=True)
...
def line_to_line(gdf, target, tolerance, extension):
    ...
    df = df.drop(ends)
    final = gpd.GeoSeries(new_geoms).explode(index_parts=True).reset_index(drop=True)
    return df.append(
