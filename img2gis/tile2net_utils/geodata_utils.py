...
def unary_multi(gdf):
	...
	"""
	# ST_makeValid(gdf.unary_union.geom)
	# print('gdf.unary_union.geoms: ',gdf.unary_union.geoms)
 
	# print(gdf.apply(geometry_reviewer))
	if gdf.unary_union.geom_type == 'MultiPolygon':
...
def buffer_union(gdf, buff, simp1, simp2):
	...
	"""
	# print('before buffer, gdf.geometry.is_valid:', gdf.geometry.is_valid)
	gdf.geometry = gdf.geometry.buffer(buff, join_style=2, cap_style=3)
	# print('after buffer, gdf.geometry.is_valid:', gdf.geometry.is_valid)
...
def buffer_union_erode(gdf, buff, erode, simp1, simp2, simp3):
	...
	gdf_buff.geometry = gdf_buff.geometry.set_crs(3857)
	gdf_buff.geometry = gdf_buff.geometry.simplify(simp3)
	return gdf_buff
...
def buff_dfs(gdf):
	"""
	union and buffer the polygons,
	to create continuous polygons into a GeoDataFrame.
	...
	buffergdf = buffer_union_erode(gdf, 0.3, -0.3, 0.2, 0.3, 0.3)
	buffergdf.geometry = buffergdf.geometry.set_crs(gdf.crs)
